...
class ReporteCompletoAsistenciasExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths, WithTitle
{
    protected $data;
        ...
        foreach (range(2, $lastRow) as $row) {
            ...
            if ($tipo === 'SIN ASISTENCIA') {
                $sheet->getStyle("A{$row}:P{$row}")->applyFromArray([
                    'fill' => [
                        'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
                        'startColor' => ['rgb' => 'FFE6E6'], // Rojo claro para ausencias
                    ],
                ]);
                ...
                if ($estado === 'EN CURSO') {
                    $sheet->getStyle("A{$row}:P{$row}")->applyFromArray([
                        'fill' => [
                            'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
                            'startColor' => ['rgb' => 'E6F3FF'], // Azul claro para en curso
                        ],
                    ]);
                }
            }
